...
char text[] = "THE DUCK JUMPS OUT THE WINDOW. 'CAUSE Y NOT. ^&%&*(%*";
uint32_t corlors[] = {
  M.Color(255,50,50),
  M.Color(255,100,50),
  M.Color(255,200,50),
...
void loop() {
  ...
  printStr(text, corlors, -(++t));
  if (t > (signed)sizeof(text) * 6-6)t = -32;
  delay(10);
}
//7-0
...
void setxy(uint32_t col, int x, int y) {
  if (x > 31)return;
...
void printStr(char str[], uint32_t col[], int x) {
  ...
  setAll(0);
  while (str[r]) {
    printChar(str[r], col[(round(t/2)+r*3+32)%8], x + r * 6);
    r++;
  }
  M.show();
}
void setAll(uint32_t col) {
  for (int i = 0; i < 256; i ++)M.setPixelColor(i, col);
}
