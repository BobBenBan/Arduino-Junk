...
void loop() {
  ...
  printStr(str, M.Color(0, 255, 100), -place);
  place+=1;
  if(place>(signed)sizeof(str)*7-7) place = -32;
  delay(10);
}
//7-0
bool getBit(uint8_t val, uint8_t b) {
  return val & 128 >> b;
}
void clearM() {
  for (int i = 0 ; i < 256; i ++)M.setPixelColor(i, 0);
}
void setxy(uint32_t col, uint16_t x, int y) {
  if (x > 31)col = 0;
  //x=31-x;
  //y-7-y;
  M.setPixelColor(x * 8 + (x % 2 ? y : 7 - y), col);

}
void printChar(uint8_t cha, uint32_t col, int x) {
  for (int i = 0 ; i < 5; i++) {
    for (int j = 0 ; j < 8; j++) {
      setxy(getBit(chars[i + cha * 5], j) ? col : 0, x + i, j);
    }
  }
}
void printStr(char str[], uint32_t col, int x) {
  int t = 0;
  clearM();
  while (str[t]) {
    printChar(str[t], col, t * 7 + x);
    t++;
  }
  M.show();
}
